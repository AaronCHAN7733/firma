...
function Componentes() {
  ...
  const handleSaveComponente = async () => {
    if (descripcionComponente && codigoComponente) {
      if (isEditing) {
        ...
        await updateDoc(componenteDoc, { descripcion: descripcionComponente, codigoComponente });
        const updatedComponentes = componentes.map((componente) =>
          componente.id === currentComponenteId
            ? { ...componente, descripcion: descripcionComponente, codigoComponente }
            : componente
        );
        setComponentes(updatedComponentes);
        Swal.fire('¡Éxito!', 'El componente ha sido actualizado correctamente.', 'success'); // Confirmación de edición
      } else {
        // Si estamos agregando un nuevo componente
        const docRef = await addDoc(collection(db, 'componentes'), { descripcion: descripcionComponente, codigoComponente });
        setComponentes([...componentes, { id: docRef.id, descripcion: descripcionComponente, codigoComponente }]);
  ...
  const handleDeleteComponente = async (id, descripcion) => {
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire('¡Eliminado!', 'El componente ha sido eliminado correctamente.', 'success');
    }
  };
  ...
  return (
    <div className="areas-container">
      ...
      {isModalOpen && (
        <div className="modal-area">
          <div className="modal-content-area">
            <h2>{isEditing ? 'Editar Componente' : 'Agregar Nuevo Componente'}</h2>
            <input
              type="text"
      ...
      <table className="areas-table">
        <thead>
          <tr>
            <th>Código del Componente</th> {/* Columna para Código del Componente */}
