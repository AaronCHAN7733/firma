...
function RevisarRequisiciones({ user }) {
  ...
  const handleEditRequisicion = () => {
    navigate("/editar-requisicion", {
      state: { requisicionId: selectedRequisicionId },
    });
                  ...
                  {requisicionesPendientes.length === 0 ? (
                    ...
                    requisicionesPendientes.map((requisicion) => (
                      <tr key={requisicion.id}>
                        <td
                          onClick={() =>
                            isMobile && toggleFolioExpansion(requisicion.id)
                          }
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => handleFirmarRequisicion(requisicion)}
                          >
                            Ver Detalles
                          </button>
      ...
      {isModalOpen && (
        <div className="modal-Auto">
          <div className="modal-content-Auto">
            ...
            <p>{motivoRechazo}</p>
            <button className="firmar-btn" onClick={cerrarModal}>
              Cerrar
            </button>
            <button className="firmar-btn" onClick={handleEditRequisicion}>
              Editar
            </button>{" "}
            {/* Botón de editar */}
          </div>
